...
function initializeDashboardDetailDialoguePage(pRuntimeInfo) {
    ...
    let sendBtn = document.getElementById("chat-control-send-btn");
    let inputTextArea = document.getElementById("chat-input-textarea");
            ...
            fetch("/api/messages/run/" + pRuntimeInfo.run_id + "?run_dir=" + pRuntimeInfo.run_dir)
                ...
                .then((data) => {
                    ...
                    window.chatSocket.on("connect", () => {
                        window.chatSocket.emit("join", {run_id: pRuntimeInfo.run_id});
                        ...
                        sendBtn.onclick = () => {
                            ...
                            window.chatSocket.emit("user_input_ready", {
                    ...
                    window.chatSocket.on("display_message", (data) => {
                    ...
                    window.chatSocket.on("enable_user_input", (data) => {
