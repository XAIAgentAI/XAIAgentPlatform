...
function applyTranslations(langObj, pendingObj, translatedObj) {
  ...
  if (!pendingObj || typeof pendingObj !== 'object') {
    // pendingObj 没有内容，直接返回原内容
    return result;
  }
  for (const key of Object.keys(pendingObj)) {
    if (typeof pendingObj[key] === 'object' && pendingObj[key] !== null) {
      // 递归处理嵌套对象
      result[key] = applyTranslations(langObj[key] || {}, pendingObj[key], translatedObj ? translatedObj[key] : undefined);
    } else if (typeof translatedObj?.[key] !== 'undefined') {
      // 只更新 pending 和 translated 都有的 key
