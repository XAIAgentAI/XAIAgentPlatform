        ...
        <div className="mt-6 flex flex-col items-center justify-center space-y-2">
          ...
          <div className="flex compress flex-wrap justify-center gap-4 max-h-[240px] overflow-y-hidden">
            {agentDescriptions[selectedAgent]?.examples?.map((example, index) => (
              <div key={index} 
                   className="bg-stone-400 hover:bg-stone-300 dark:hover:bg-zinc-700 rounded-xl dark:bg-zinc-800 px-2 py-6 text-stone-700 dark:text-neutral-500 font-light text-sm text-center flex items-center justify-center w-[210px] md:w-[140px] lg:w-[180px] min-h-[5rem]"
                   onClick={async () => {
                     if (!userName) {
                       setUserStatus(false);
                       setTimeout(() => {
