    ...
    <div className="flex catcher flex-col items-center justify-center h-[70vh] md:h-[78vh] space-y-2 mt-4 md:justify-start md:mt-12">
      ...
      <div className="relative w-full max-w-sm md:w-[80vw] md:ml-[18vw]">
        ...
        >
          {selectedAgent}
          {isAgentListOpen ? (
            <ArrowUpIcon className="w-5 h-5 mb-[2px] font-light text-stone-400 dark:text-foreground" />
          ) : (
            <ArrowDownIcon className="w-5 h-5 mb-[2px] font-light text-stone-400 dark:text-foreground" />
          )}
        </button>
        {isAgentListOpen && (
        ...
        <div className="mt-6 flex flex-col items-center justify-center space-y-2">
          ...
          <div className="flex compress flex-wrap justify-center gap-4 max-h-[240px] overflow-y-hidden">
            {agentDescriptions[selectedAgent]?.examples?.map((example, index) => (
              <div key={index} 
                   className="bg-stone-400 hover:bg-stone-300 dark:hover:bg-zinc-700 rounded-xl dark:bg-zinc-800 px-2 py-6 text-stone-700 dark:text-neutral-300 font-light text-sm text-center flex items-center justify-center w-[210px] md:w-[140px] lg:w-[180px] min-h-[5rem]"
                   onClick={async () => {
                     if (!userName) {
                       setUserStatus(false);
                       setTimeout(() => {
