    ...
    <div className="flex flex-col flex-grow bg-background w-full lg:w-[78vw] lg:ml-[22vw] xl:w-[71vw] xl:ml-[28vw] px-2 py-6" style={{ maxHeight:"75vh" }}>
      {messages.length > 0 && (
        <div className="flex justify-end items-center p-4 bg-background w-full lg:w-[71vw]">
          <button
            type="button"
            className="fixed right-[10px] top-[80px] flex items-center justify-center px-2 py-1 bg-stone-400 dark:bg-zinc-800 text-zinc-700 rounded-full"
            onClick={async () => {
              try {
          ...
          >
            <XMarkIcon className="w-4 h-4 text-background" />
          </button>
        </div>
      )}
          ...
          >
            <div
              className={`max-w-[80%] rounded-2xl px-4 py-2 ${
                message.role === 'user'
                  ? 'bg-stone-400 dark:bg-gray-600 text-white'
                  : 'text-foreground'
              }`}
            >
